...
function(rapids_cpm_load_preset_versions)
  ...
  if(NOT EXISTS "${_rapids_preset_version_file}")
    message(
      FATAL_ERROR
      "rapids_cpm can't load '${_rapids_preset_version_file}' to find package version information, verify it exists"
    )
  endif()
  ...
  get_property(
    already_loaded
    GLOBAL
    PROPERTY rapids_cpm_load_presets_${_rapids_preset_version_file}
    SET
  )
  if(already_loaded)
    return()
  ...
  foreach(index RANGE ${package_count})
    ...
    if(already_exists)
      ...
      message(
        AUTHOR_WARNING
        "RAPIDS-CMake has detected two entries for ${package_name} in ${_rapids_preset_version_file}. Please ensure a single entry as all names are cased insensitive"
      )
    else()
      set_property(GLOBAL PROPERTY rapids_cpm_${normalized_pkg_name}_json "${data}")
      set_property(
        GLOBAL
        PROPERTY rapids_cpm_${normalized_pkg_name}_json_file "${_rapids_preset_version_file}"
      )
