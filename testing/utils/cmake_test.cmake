...
function(add_cmake_test mode source_or_dir)
  ...
  elseif(EXISTS "${CMAKE_CURRENT_LIST_DIR}/${source_or_dir}")
    ...
    configure_file(
      "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/project_template.cmake.in"
  ...
  foreach(generator gen_name IN ZIP_LISTS supported_generators nice_gen_names)
    ...
    if(mode STREQUAL "config")
      add_test(
        NAME ${test_name}
        COMMAND
          ${CMAKE_COMMAND} -S ${src_dir} -B ${build_dir} -G "${generator}" ${extra_configure_flags}
    ...
    elseif(mode STREQUAL "install")
      ...
      message(
        FATAL_ERROR
        "${mode} mode not one of the valid modes (config|build|install) by add_cmake_build_test"
      )
    endif()
    ...
    if(RAPIDS_TEST_SHOULD_FAIL)
      ...
      set_tests_properties(
        ${test_name}
        PROPERTIES FAIL_REGULAR_EXPRESSION "${RAPIDS_TEST_SHOULD_FAIL}"
      )
    else()
